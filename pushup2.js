...
var list = function(_filelist){
  ...
  for(var i=0;i<_filelist.length;i++){
    ...
    flist +=`<form action='/delete_process' method='post' onsubmit="return confirm('delete?')">
                <input type='hidden' name='date' value=${_filelist[i]}>
                <input type='submit' value='delete'>
            </form>`
    var data = fs.readFileSync(`./pushuplist/${_filelist[i]}`,'utf8');
    flist +=`<p>${data}</p>`;
  }
  console.log(flist);
  //flist += '</ul>';
...
var app = http.createServer(function(request,response){
  ...
  if(pathname==='/'){
    if(title===undefined){
      ...
      response.writeHead(302,{Location: `/update?id=${qs.escape(title)}`});
      response.end();
    }
